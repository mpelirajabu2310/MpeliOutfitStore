    ...
    <section class="login-panel" aria-label="Admin login" data-i18n-aria-label="aria.adminLogin">
      <form class="login-card" id="loginForm">
        ...
        <div class="logo-lockup">
          ...
          <div>
            ...
            <small data-i18n="brand.adminWorkspace">Admin Workspace</small>
          </div>
        </div>
      ...
      <form class="login-card setup-card hidden" id="ownerSetupForm">
        <div class="logo-lockup">
          ...
          <div>
            <strong>Mpeli Outfit Store</strong>
            <small data-i18n="auth.firstOwnerSetup">First owner setup</small>
          </div>
        </div>
  ...
  <div class="app-shell hidden" id="appShell">
    <aside class="sidebar">
      <div class="sidebar-brand">
        ...
        <div>
          <strong>Mpeli Outfit Store</strong>
          <small data-i18n="brand.boutiqueAdmin">Boutique Admin</small>
        </div>
      </div>
      <nav class="side-nav" aria-label="Main navigation" data-i18n-aria-label="aria.mainNavigation">
        ...
        <button class="nav-item owner-only" data-page="expenses" data-i18n="nav.expenses">Expenses</button>
    ...
    <section class="workspace">
      <header class="topbar">
        ...
        <div class="admin-profile owner-only">
      ...
      <main class="page active" id="dashboard">
        ...
        <section class="stats-grid">
          ...
          <article class="stat-card owner-only"><span data-i18n="stats.dailyProfit">Daily Profit</span><strong id="dailyProfit">TSH 0</strong><small data-i18n="stats.profitToday">Profit today</small></article>
          <article class="stat-card owner-only"><span data-i18n="stats.monthlyProfit">Monthly Profit</span><strong id="monthlyProfit">TSH 0</strong><small data-i18n="stats.grossProfitMonth">Gross profit this month</small></article>
        ...
        <section class="dashboard-grid">
          ...
          <article class="panel visual-card owner-only">
            <div class="visual-placeholder" aria-hidden="true"></div>
            <div>
              <span data-i18n="dashboard.storeFloor">Overview</span>
              <strong id="dashboardSummaryText" data-i18n="dashboard.storeFloorEmpty">Add products and record sales to see live performance here.</strong>
            </div>
          </article>
        </section>
      ...
      <main class="page owner-only" id="reports">
        ...
        <section class="report-grid">
          ...
          <article class="panel owner-only"><h3 data-i18n="reports.profitAnalytics">Profit Analytics</h3><strong id="reportProfit">TSH 0</strong><p id="reportProfitNote" class="report-note" data-i18n="dashboard.noChartData">No sales data available yet.</p></article>
        </section>
        <article class="panel">
      ...
      <main class="page owner-only" id="expenses">
        <div class="page-heading">
          <div><p class="eyebrow" data-i18n="expenses.eyebrow">Cost Tracking</p><h2 data-i18n="nav.expenses">Expenses</h2></div>
          <button class="gold-button" data-i18n="expenses.recordExpense">Record expense</button>
        </div>
        <section class="expense-layout">
          <article class="panel expense-form">
            <h3 data-i18n="expenses.recordShopExpenses">Record shop expenses</h3>
            <label class="sr-only" for="expenseTitle" data-i18n="expenses.titlePlaceholder">Expense title</label>
            <input placeholder="Expense title" id="expenseTitle" data-i18n-placeholder="expenses.titlePlaceholder" />
            <label class="sr-only" for="expenseCategory" data-i18n="expenses.categoryLabel">Expense category</label>
            <select id="expenseCategory" aria-label="Expense category" data-i18n-aria-label="expenses.categoryLabel"><option value="Rent" data-i18n="expenseCategory.rent">Rent</option><option value="Utilities" data-i18n="expenseCategory.utilities">Utilities</option><option value="Packaging" data-i18n="expenseCategory.packaging">Packaging</option><option value="Marketing" data-i18n="expenseCategory.marketing">Marketing</option><option value="Supplier Transport" data-i18n="expenseCategory.supplierTransport">Supplier Transport</option></select>
            <label class="sr-only" for="expenseAmount" data-i18n="table.amount">Amount</label>
            <input placeholder="Amount" id="expenseAmount" type="number" min="0" step="0.01" data-i18n-placeholder="table.amount" />
            <button class="gold-button full" id="saveExpenseButton" data-i18n="expenses.saveExpense">Save expense</button>
          </article>
          <article class="panel">
            <h3 data-i18n="expenses.tracking">Daily and monthly expense tracking</h3>
            ...
            <div class="expense-row"><span data-i18n="expenses.topCategory">Top Category</span><strong data-i18n="expenseCategory.rent">Rent</strong></div>
          </article>
        </section>
      </main>
      <main class="page owner-only" id="settings">
        ...
        <section class="settings-grid">
          <article class="panel settings-card">
            ...
            <label class="sr-only" for="shopName" data-i18n="settings.shopName">Shop name</label>
            <input id="shopName" aria-label="Shop name" data-i18n-aria-label="settings.shopName" />
            <label class="sr-only" for="shopAddress" data-i18n="settings.shopAddress">Shop address</label>
            <input id="shopAddress" aria-label="Shop address" data-i18n-aria-label="settings.shopAddress" />
            <label class="sr-only" for="shopPhone" data-i18n="settings.shopPhone">Shop phone</label>
          ...
          <article class="panel settings-card">
            ...
            <label class="sr-only" for="adminName" data-i18n="settings.adminName">Admin full name</label>
            <input id="adminName" aria-label="Admin full name" data-i18n-aria-label="settings.adminName" />
            <label class="sr-only" for="adminEmail" data-i18n="settings.adminEmail">Admin email</label>
            <input id="adminEmail" aria-label="Admin email" data-i18n-aria-label="settings.adminEmail" />
            <label class="sr-only" for="adminPassword" data-i18n="settings.adminPassword">Admin password</label>
          ...
          <article class="panel settings-card">
            ...
            <label><span data-i18n="settings.lowStockThreshold">Low stock threshold</span><input type="number" id="lowStockThreshold" min="1" step="1" value="5" /></label>
            <label><span data-i18n="settings.receiptFooter">Receipt footer</span><input type="text" id="receiptFooter" /></label>
          </article>
        </section>
